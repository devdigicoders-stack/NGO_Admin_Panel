...
const DonationQueriesManagement = () => {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      const [listRes, statsRes] = await Promise.all([
        fetch(`${API_BASE}/donation-queries?${params}`, { credentials: 'include' }),
        fetch(`${API_BASE}/donation-queries/stats`, { credentials: 'include' }),
      ]);

      const listJson = await listRes.json();
      const statsJson = await statsRes.json();

      if (listJson.success) setQueries(listJson.data || []);
      if (statsJson.success) setStats(statsJson.data || {});
    } catch (err) {
      console.error(err);
      toast({ title: 'Failed to load queries', status: 'error', duration: 3000 });
    } finally {
      setLoading(false);
    }
  ...
  const handleSave = async () => {
    ...
    setSaving(true);
    try {
      const res = await fetch(`${API_BASE}/donation-queries/${selected.id}`, {
        method: 'PATCH',
      ...
      toast({ title: 'Query updated', status: 'success', duration: 2500 });
      onClose();
      fetchData();
    } catch (err) {
  ...
  const handleDelete = async () => {
    ...
    try {
      const res = await fetch(`${API_BASE}/donation-queries/${itemToDelete.id}`, {
        method: 'DELETE',
        credentials: 'include',
      });
      const json = await res.json();
      ...
      toast({ title: 'Query deleted', status: 'success', duration: 2500 });
      onDeleteClose();
    ...
    <VStack spacing={6} align="stretch" w="full">
      <Flex justify="space-between" align={{ base: 'stretch', md: 'center' }} direction={{ base: 'column', md: 'row' }} gap={4}>
        <Box>
          ...
          <Text fontSize="sm" color={mutedColor}>
            User messages from the About page donation form
          </Text>
        </Box>
          ...
          <Box overflowX="auto">
            <Table variant="simple" size="md">
              ...
              <Tbody>
                {queries.length === 0 ? (
                  <Tr>
                    <Td colSpan={7} textAlign="center" py={12} borderColor={tdBorder}>
                      <VStack spacing={2}>
                  ...
                  queries.map((q, index) => (
                    ...
                    >
                      ...
                      <Td borderColor={tdBorder} py={3.5} maxW="220px">
                        <HStack spacing={2}>
                          <Icon as={FiMail} color="#03735F" flexShrink={0} />
                      ...
                      <Td borderColor={tdBorder} py={3.5} display={{ base: 'none', md: 'table-cell' }}>
                        <HStack spacing={2}>
                          <Icon as={FiPhone} color={mutedColor} flexShrink={0} />
                ...
                <VStack align="stretch" spacing={5}>
                  ...
                  <Box>
                    <HStack justify="space-between" mb={3}>
                      ...
                      <Button
                        as="a"
                        href={`mailto:${selected.email}?subject=Re: Your donation query`}
                        size="xs"
      ...
      <Modal isOpen={isDeleteOpen} onClose={onDeleteClose} isCentered>
        ...
        <ModalContent borderRadius="2xl">
          ...
          <ModalCloseButton />
          <ModalBody>
            <Text>Delete query from <strong>{itemToDelete?.email}</strong>? This cannot be undone.</Text>
          </ModalBody>
          <ModalFooter>
...
export default DonationQueriesManagement;
